...
rebuilding presentation and interaction per screen** (see `intent.md`). Every
...
1. **Boundary.** *Accept* presentation or interaction a host would otherwise
...
2. **Less for more.** *Accept* when the change removes more host code or
   decisions than the API it adds. *Refuse* a prop, variant or component with
   one caller. *Rethink* when a composition of existing exports already does
   the job.
...
3. [ ] **Prove the framework with the Purchase Orders and Sales sample pages.**
   ...
   commit it tested. Serves: Objective 2. Needs: a `[UI request]` issue agreed
...
4. [ ] **Modal on the native `<dialog>` element.** Accept: `Modal` opens with
   ...
   tests pass unmodified. Serves: Objective 1 (closes the `aria-modal`-only
...
6. [ ] **Docs cannot drift from components.** Accept: a unit test fails when a
   ...
   planted defect in its PR. Serves: Objective 2.

Each increment needs an acceptance-to-test mapping and independent review.
