...
| `Finance.tsx` | None — ROADMAP item 34 ("Finance" slice, issue #16), not part of the M7 Slice-numbered sequence above | The Finance module's first real screen, "Overview" — fills `NAV.Finance`'s pre-existing placeholder from M6. A module-area card grid (one real figure: live accounts-receivable total from `state.invoices`) plus a "Cash flow, last 12 months" line `Chart` — disclosed sample data, since the store has no seeded payment history to aggregate a real trend from |
| `BiExplore.tsx` | None — ROADMAP item 34 ("BI explore" slice, issue #16), not part of the M7 Slice-numbered sequence above | A real pivot-result screen filling `NAV.BI`'s pre-existing "Explore" placeholder from M6: a "Pivot by" `Dropdown` re-slices the same live `stockLevels` data `Inventory.tsx` already charts, by warehouse or by material, into a bar `Chart` plus its own result `Table`. Deliberately does NOT add a `'pie'` `Chart` variant (issue #16's own "Pie" ask) — this screen is the union's only named consumer anywhere in this repo, short of the Objective 1/3 two-consumer bar; see the file's own header comment |
