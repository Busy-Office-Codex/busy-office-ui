...
Known gap, not yet closed: this recipe has only been applied to `ListReport.tsx`, `RecordDetail.tsx`, `Delivery.tsx` and `Inventory.tsx` so far. One literal in `Delivery.tsx` (the deliveries-table selected-row tint, `#eff6ff`) was deliberately left as a raw hex rather than force-mapped — no `color.*` token in `lightPalette` matches that exact value, and every other `examples/*.tsx` page sharing this same selected-row pattern (e.g. `Requisitions.tsx`, `Companies.tsx`) has the identical un-migrated literal, so this is a real gap in the token scale, not something this page-level pass alone can close. Every other `examples/*.tsx` sample page still has its own raw hex/spacing literals (a pre-existing, disclosed condition since issue #19 shipped — see `docs/Theme.md`); each is a candidate for the same 3-step pass.
