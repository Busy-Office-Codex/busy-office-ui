...
Chrome density matches the ERP skeleton reference rather than the `Button`/`Chip` component scale: app-strip nav items are 13px text pills (padding `6px 12px`, weight 600 only when active), the palette trigger is a 520×36px search-bar look, and the dock launcher/pinned tiles are 44×44px icon squares — all plain styled elements, not `Button`, because `Button` has no compact size to produce them. Pinned tiles show a single uppercase initial as their glyph (`aria-label`/`title` still carry the full name); there is no Icon component yet to render a real glyph instead. These chrome elements get the browser's default focus outline, not the branded `:focus-visible` ring `Button` provides, and have no `:hover` treatment (both are CSS states plain inline styles can't express) — known, disclosed gaps, not oversights.
