...
Known gaps, disclosed rather than silently dropped: this covers the `color.*` token group only. `examples/*.tsx`'s own sample screens use plain inline hex literals throughout (never StyleX), so they will not visually follow dark mode even though every real `src/` component does. `Button`'s danger-hover fill (a static `rgba()` derived from the light-mode danger hex) and `Shell`'s `glass`/`shadow` token groups are similarly static values that won't re-tint under dark mode — theming those is a separate, larger token group outside this component's scope.
