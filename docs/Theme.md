...
Known gaps, disclosed rather than silently dropped: this covers the `color.*` token group only. `examples/*.tsx`'s own sample screens use plain inline hex literals throughout (never StyleX), so they will not visually follow dark mode even though every real `src/` component does. `Modal`'s backdrop/panel and `Dropdown`'s menu popover both read the `glass.*` token group (M10 Dropdown scoring, issue #24: re-verified directly — `glass` is a plain `stylex.defineVars` with no `@media (prefers-color-scheme: dark)` branch at all, unlike `color.*`), so both stay a frozen light-glass panel regardless of theme; `shadow.*` is the same shape. `Shell`'s own chrome has a related but distinct gap — its command bar/app-strip use hardcoded translucent `rgba()` literals directly, not the `glass` tokens at all. Theming `glass`/`shadow` for real is a separate, larger token-group decision outside this component's scope — this file only names the gap, not a plan to close it.
