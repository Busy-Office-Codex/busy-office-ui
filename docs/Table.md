...
A `TableRow` with an `onClick` is real, keyboard-operable interaction, not just a mouse cue (M10, issue #24) — it becomes tab-focusable, Enter/Space triggers the same click, and it renders a visible `:focus-visible` ring (inset, unlike `Card`'s outset one, since a row has no independent box separation from its neighbors). This is the row-selection pattern 10 real `examples/*.tsx` screens already use (list+detail, `docs/layouts.md`): pass `onClick` plus a background driven by `color.bgSelected` when the row is the current selection. Pass `selected` too to expose that same state as `aria-selected` — **the programmatic channel only**: unlike `Card`'s identical `selected` (a real `aria-pressed` plus a non-colour checkmark badge), no non-colour visual cue exists for a selected row yet; picking and building one is real, disclosed, not-yet-designed follow-up work, not claimed solved here.
...
<Density value="compact">
  <Table>
    ...
    <TableBody>
      <TableRow onClick={() => {}} selected style={{ backgroundColor: color.bgSelected }}>
